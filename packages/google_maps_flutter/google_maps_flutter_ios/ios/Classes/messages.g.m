...
+ (instancetype)makeWithId:(NSInteger )id {
  FGMPlatformBitmapRegisteredMapBitmap* pigeonResult = [[FGMPlatformBitmapRegisteredMapBitmap alloc] init];
  pigeonResult.id = id;
  return pigeonResult;
}
+ (FGMPlatformBitmapRegisteredMapBitmap *)fromList:(NSArray<id> *)list {
...
void SetUpFGMImageRegistryApi(id<FlutterBinaryMessenger> binaryMessenger, NSObject<FGMImageRegistryApi> *api) {
  SetUpFGMImageRegistryApiWithSuffix(binaryMessenger, api, @"");
}

void SetUpFGMImageRegistryApiWithSuffix(id<FlutterBinaryMessenger> binaryMessenger, NSObject<FGMImageRegistryApi> *api, NSString *messageChannelSuffix) {
  messageChannelSuffix = messageChannelSuffix.length > 0 ? [NSString stringWithFormat: @".%@", messageChannelSuffix] : @"";
  /// Adds a bitmap to the cache.
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:[NSString stringWithFormat:@"%@%@", @"dev.flutter.pigeon.google_maps_flutter_ios.ImageRegistryApi.addBitmapToCache", messageChannelSuffix]
        binaryMessenger:binaryMessenger
        codec:FGMGetMessagesCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(addBitmapToCacheId:bitmap:error:)], @"FGMImageRegistryApi api (%@) doesn't respond to @selector(addBitmapToCacheId:bitmap:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray<id> *args = message;
        NSInteger arg_id = [GetNullableObjectAtIndex(args, 0) integerValue];
        FGMPlatformBitmap *arg_bitmap = GetNullableObjectAtIndex(args, 1);
        FlutterError *error;
        [api addBitmapToCacheId:arg_id bitmap:arg_bitmap error:&error];
        callback(wrapResult(nil, error));
      }];
    } else {
      [channel setMessageHandler:nil];
  ...
  /// Retrieves a bitmap from the cache.
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:[NSString stringWithFormat:@"%@%@", @"dev.flutter.pigeon.google_maps_flutter_ios.ImageRegistryApi.removeBitmapFromCache", messageChannelSuffix]
        binaryMessenger:binaryMessenger
        codec:FGMGetMessagesCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(removeBitmapFromCacheId:error:)], @"FGMImageRegistryApi api (%@) doesn't respond to @selector(removeBitmapFromCacheId:error:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray<id> *args = message;
        NSInteger arg_id = [GetNullableObjectAtIndex(args, 0) integerValue];
        FlutterError *error;
        [api removeBitmapFromCacheId:arg_id error:&error];
        callback(wrapResult(nil, error));
      }];
    } else {
      [channel setMessageHandler:nil];
  ...
  /// Clears the bitmap cache.
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:[NSString stringWithFormat:@"%@%@", @"dev.flutter.pigeon.google_maps_flutter_ios.ImageRegistryApi.clearBitmapCache", messageChannelSuffix]
        binaryMessenger:binaryMessenger
        codec:FGMGetMessagesCodec()];
    if (api) {
      ...
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        FlutterError *error;
        [api clearBitmapCacheWithError:&error];
        callback(wrapResult(nil, error));
      }];
    } else {
      [channel setMessageHandler:nil];
